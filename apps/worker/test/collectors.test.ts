...
import { articleDedupeKey, legacyArticleSimilarityKey } from "../src/repository.js";
...
describe("RSS collection policy", () => {
  ...
  it("uses versioned source and canonical URL identity without collapsing title/hour matches", () => {
    const base = {
      id: "id",
      source: "nrk" as const,
    ...
    expect(articleDedupeKey(base)).toMatch(/^article-url-v2:/);
    expect(articleDedupeKey(base)).toBe(
      articleDedupeKey({
        ...base,
        title: "En helt ny tittel",
    ...
    expect(articleDedupeKey(base)).not.toBe(
      articleDedupeKey({
        ...
        title: "BRANN i Bymarka",
        publishedAt: "2026-05-26T12:45:00Z",
      }),
    );
    expect(legacyArticleSimilarityKey(base)).toBe(
      legacyArticleSimilarityKey({
        ...base,
        id: "id-2",
        url: "https://example.test/2",
        title: "BRANN i Bymarka",
