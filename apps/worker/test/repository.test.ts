...
describe("WorkerRepository", () => {
  it("refreshes stored article metadata without replacing situation linkage", async () => {
    ...
    expect(query.mock.calls[0]?.[0]).not.toContain("payload ? 'coverageBundle'");
  ...
  it("preserves same-title same-hour articles on different canonical URLs", async () => {
    ...
    expect(canonical.map(({ id }) => id)).toEqual(["t_a-z", "t_a-a"]);
  });
