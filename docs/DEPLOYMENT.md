...
The deployment playbook now automatically verifies live health, promoted worker container state, a `worker_cycle_metrics.latest` cycle completed after candidate promotion began, `state='ok'` traffic, DATEX/datex_travel_time and Entur `source_health` rows, source-item query sanity and the invariant that TravelTime traffic-pulse rows are not written to `source_items`. The source-health gates are row/state checks rather than per-source freshness checks because collectors are external, scheduled and sometimes cooldown-gated; Trafikkdata, for example, may honestly say the poll was skipped because the last successful fetch is still recent. For manual follow-up after deploying DATEX ingestion, verify source status, worker stability, source-health timestamps, persisted official traffic rows and TravelTime traffic-pulse rows:
