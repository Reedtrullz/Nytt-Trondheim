...
5. Ansible checks out the exact CI-verified commit, installs/configures backup tooling, pulls/builds fresh Docker images, verifies an encrypted pre-migration backup, applies locked transactional migrations, readiness-checks a canary API container on a separate localhost port, promotes API/worker only after the canary is ready, validates and reloads Caddy, then runs production readiness, worker, source-health and source-item sanity checks. Docker uses `/health/live` so a temporary database outage does not restart a healthy API process; canary, promotion and rollback validation use `/health/ready` to require working storage. Readiness coalesces concurrent checks, briefly caches success and uses a short-lived dedicated client with bounded connection time, a PostgreSQL-enforced statement timeout and a slightly longer client read deadline. A failed HTTP probe therefore does not leave its database query running, queue work in the application pool or create unbounded readiness connections. `/health` remains a backwards-compatible readiness alias. The current production API/worker stay up through backup and canary, but migrations run before canary against the production database, so application migrations must be expand/contract-compatible with the previous release; destructive schema changes must be split into a later deploy or paired with an explicit restore/rollback procedure.
...
2. **Review:** observe seven completed v2 shadow generations within 24 hours. Require clean storage parity, zero integrity errors, a passing exact-SHA CI run (including `npm run check:coverage-matcher`), and review every changed bundle. Storage parity compares the normalized v2 candidate with its isolated legacy-shaped serialization from the exact same generation, identified by `state='superseded'`, `matcher_version='v1'`, and `legacy_generation_id`; it does not compare v2 grouping decisions with the still-public v1 matcher. Public `state='legacy'` rows are excluded, so intentional matcher improvements remain reviewable without weakening corruption detection or leaking into the public projection. Active correction membership is intentionally excluded from storage-parity comparison and is validated separately. Record only the UUID of the latest completed owner-reviewed shadow generation in the non-secret production environment variable `COVERAGE_V2_OWNER_REVIEWED_GENERATION_ID`; do not store review notes there.
...
`base_parity_mismatches`, `dangling_members`, stable-row count/association/membership checks, and invalid-primary/integrity checks must be zero. Public legacy rows and paired rows from another generation are deliberately absent from this comparison. `correction_conflicts` is a separate review diagnostic and does not mean that an intentional active split made storage parity dirty. Failed candidate transactions preserve both the public legacy projection and the previous normalized projection. CI applies the schema twice, executes older-review/zero-row/historical-drop/paired-mismatch/stable-count controls, then executes the same production promotion SQL Ansible uses. Its actual `WorkerRepository`/`PgStore` lifecycle proves rollback of public and paired legacy mutation, advisory-lock exclusion, stable-row readiness, split/effective feed/effective audit/undo, and legacy readability against PostgreSQL. Superseded completed generations older than 30 days are pruned after successful generation writes together with their paired parity rows; current active/shadow generations, correction rows, and generations referenced by corrections are preserved.
