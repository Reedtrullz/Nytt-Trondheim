...
The deployment playbook now automatically verifies live health, promoted worker container state, `state='ok'` traffic, DATEX/datex_travel_time and Entur `source_health` rows, source-item query sanity and the invariant that TravelTime traffic-pulse rows are not written to `source_items`. These source-health gates are row/state checks rather than freshness checks because collectors are external, scheduled and sometimes cooldown-gated; Trafikkdata, for example, may honestly say the poll was skipped because the last successful fetch is still recent. Do not require a new end-of-cycle `collector_runs` row during deployment. For manual follow-up after deploying DATEX ingestion, verify source status, worker stability, source-health timestamps, persisted official traffic rows and TravelTime traffic-pulse rows:
