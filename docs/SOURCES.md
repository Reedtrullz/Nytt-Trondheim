...
`source_items` is the current source projection. Every distinct admitted capture is also retained
append-only in `source_item_captures`, keyed by provider and capture hash. The capture ledger keeps
first-seen, upstream publication, optional upstream-update and collection clocks separate, together
with the raw and normalized payloads available at that ingestion boundary. Repeated identical
captures are idempotent. Current projection updates must never rewrite historical capture rows, and
the capture table is not exposed by public or ordinary owner list APIs.

Coverage bundles are not a collected source. They are worker-derived article grouping decisions stored as stable bundle rows plus immutable generation-scoped versions, members, and edges for owner-only Command Center review at `/command/dekning`. Persisted generation membership is the uncorrected matcher base; owner rejected pairs are a current-generation read overlay and separate conflict diagnostic, not new evidence. They may reference article ids and source labels, but they do not write raw upstream payloads, do not require a source contract, and must not be mirrored into `source_items`.

Matcher versions and owner corrections are derived decisions, not collected sources. Shadow edges, evaluation labels, exact-pair split/undo records, and sanitized correction exports must never be written to `source_items`, used as automatic situation evidence, or change source reliability. A correction changes only the effective grouping projection; it cannot activate, verify, dismiss, or otherwise mutate a situation.

Normalized edges retain bounded positive incident-evidence labels for owner audit. Only an accepted, conflict-free strong incident edge directly connecting an official article and a newsroom article may derive a public coverage verification badge. Reviewable, weak, topical, corrected-conflict, or indirect bridge edges cannot do so.

Promotion from shadow to active changes which derived grouping projection feed and audit read; it does not promote grouping or correction data into provenance. Neither generated membership, correction conflicts, owner review, nor a split/undo may satisfy an automatic situation-activation rule.
...
- `/vaer` reads MET Locationforecast (`https://api.met.no/weatherapi/locationforecast/2.0/documentation`) and MET Nowcast (`https://api.met.no/weatherapi/nowcast/2.0/documentation`) at request time for local Trondheim forecast zones. These requests use MET's documented User-Agent and cache guidance (`https://api.met.no/doc/GettingStarted`), short server-side caching, and honest degraded states when a product is unavailable. Forecast products are displayed as public weather context only; they do not create `source_items`, `official_events`, or situations.
...
- `/vaer` keeps NVE flood (`https://api.nve.no/doc/flomvarsling/`) and landslide (`https://api.nve.no/doc/jordskredvarsling/`) warnings source-labeled as warning/preparedness context. NVE warnings can affect local weather risk copy and map context, but weather-derived thresholds remain `preparedness_context` and cannot activate incidents.
...
- The authenticated travel-planner API `/api/map/travel-plan?from=...&to=...` resolves free-text places with Nominatim/OpenStreetMap inside a Trøndelag travel service area (`62.2..64.7`, `8.0..12.4`), accepts direct coordinates in either `lat,lng` or `lng,lat` order when they fall inside that service area, requests an OSRM driving route, and falls back to a straight corridor if routing is unavailable. It filters bounded stored TrafficInfo/DATEX map events within 1.5 km of the route and Entur vehicles/alerts within 1.2 km. It also performs request-time Entur Journey Planner v3 trip search for bus, tram, rail and water options with short timeout, cache, rate-limit and failure backoff handling. `/api/map/travel-suggestions` performs request-time Entur Geocoder v3 autocomplete for stops, streets, places and points of interest inside the same Trøndelag travel area. `/api/map/public-transport/departures` performs a request-time Entur Journey Planner v3 departure-board lookup for nearby stops around Trondheim sentrum or a supplied coordinate. These route, trip, autocomplete and departure-board lookups are transient recommendation context; they do not ingest new upstream traffic data and do not create `source_items`, `official_events` or `situations`.
...
- Entur Journey Planner v3 `situations(codespaces:["ATB"])` supplies official public-transport service alerts. This is separate from request-time trip search. Service alerts are stored in `public_transport_service_alerts` and mirrored to `source_items` provider `entur`, kind `official_event`; trip-search responses are not stored and are not mirrored. Neither surface is an automatic situation activator in this release.
...
The worker creates preliminary situations only for explicit incident categories and a shared specific place identifier. A general Trondheim mention is valid for feed relevance, but not for linking independent stories into one incident. Open situations accept timely matching reporting and matching official closure updates; resolved or dismissed cases cannot absorb a later new event at the same place. Optional DeepSeek experiments may enrich an already qualifying group with cited text; they do not bypass deterministic activation requirements and are disabled in production deploys.

When `DEEPSEEK_ANALYSIS_ENABLED=true` is set outside the production playbook, DeepSeek may receive compact summaries of currently open situations so it can suggest relevant follow-up stories, likely bundle candidates, category/topic corrections and relevance notes. The prompt and payload are deliberately bounded, and the worker treats malformed or empty DeepSeek output as a degraded optional enrichment rather than a source-ingest failure. Every consumed hint must cite literal public article snippets. Situation-progress hints may add public articles, evidence snippets and timeline context to an existing open situation, but they cannot create situations, reopen resolved history, mirror anything into `source_items`, or turn a source into official evidence. Bundle/category/relevance hints remain operational analysis until deterministic code adopts them.
