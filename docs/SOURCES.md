...
- The authenticated travel-planner API `/api/map/travel-plan?from=...&to=...` resolves free-text places with Nominatim/OpenStreetMap inside the Trondheim service bounds (`63.25..63.62`, `10.05..10.85`), accepts direct coordinates in either `lat,lng` or `lng,lat` order when they fall inside that service area, requests an OSRM driving route, and falls back to a straight corridor if routing is unavailable. It filters bounded stored TrafficInfo/DATEX map events within 1.5 km of the route and Entur vehicles/alerts within 1.2 km. It also performs request-time Entur Journey Planner v3 trip search for bus, tram, rail and water options with short timeout, cache, rate-limit and failure backoff handling. `/api/map/public-transport/departures` performs a request-time Entur Journey Planner v3 departure-board lookup for nearby stops around Trondheim sentrum or a supplied coordinate. These route, trip and departure-board lookups are transient recommendation context; they do not ingest new upstream traffic data and do not create `source_items`, `official_events` or `situations`.
