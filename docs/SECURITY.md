...
- Viewer users can read the main news, situation, traffic and weather surfaces. Owner-only APIs still protect the Command Center, saved items, source audit/source linking, private workspace mutations, notes, drawings, attachments and exports. The route-planner API is authenticated; free-text origin/destination queries are sent server-side to Nominatim/OpenStreetMap for geocoding, OSRM for routing, and resolved labels/coordinates are sent to Entur Journey Planner v3 for transient public-transport trip search. Nytt does not persist these route-planner queries, and Entur trip results are traveller context rather than source/evidence ledger data.
...
- Private workspace content is excluded from AI processing. Production deploys force DeepSeek off; any
...
Optional: `NYTT_SMTP_PORT`, `NYTT_SMTP_SECURE`, `NYTT_SMTP_USER`, `NYTT_DATEX_ENDPOINT`; repository variables `NYTT_POLITILOGGEN_ENABLED`, `NYTT_ENTUR_CLIENT_NAME`, `NYTT_ENTUR_CODESPACES`, `NYTT_ENTUR_VEHICLE_BOUNDS`, and `NYTT_DEPLOY_ENABLED`.
